...
def draw_polygons(polygons, screen, color):
    ...
    while i < len(polygons) - 1:
        if (calculate_normal(polygons, i)[2] > 0):
            ...
            point2 = polygons[i + 1]
            point3 = polygons[i + 2]
            draw_line(int(point1[0]), int(point1[1]), int(point2[0]), int(point2[1]), screen, color)
            draw_line(int(point2[0]), int(point2[1]), int(point3[0]), int(point3[1]), screen, color)
            draw_line(int(point3[0]), int(point3[1]), int(point1[0]), int(point1[1]), screen, color)
        i += 3
...
def add_sphere(polygons, cx, cy, cz, r, step):
    ...
    for lat in range(lat_start, lat_stop):
        for longt in range(longt_start, longt_stop):
            ...
            point3 = points[(index + step) % len(points)]
            point4 = points[(index + 1 + step) % len(points)]
